...
struct ModuleParams {
  ...
  std::string to_string() const {
    ...
    result += "  states_update_default_rate: " + std::to_string(states_update_default_rate) + "\n";

    result += "  states_custom_rates:\n";
    for(const auto &entry : states_custom_rates) {
      result += "    - state_name: " + entry.first + ", rate: " + std::to_string(entry.second) + "\n";
    }

    result += "  parameters_default_values:\n";
    for(const auto &entry : parameters_default_values) {
      result += "    - param_name: " + entry.first + ", default_value: " + entry.second + "\n";
    }
