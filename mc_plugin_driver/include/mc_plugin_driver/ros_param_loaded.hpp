...
class McDriverParamsLoader
{
 public:
    ...
    try {
      ...
      if (!config["states_update_default_rate"]) {
        return Status::KeyError(
          "No 'states_update_default_rate' specified in global YAML file");
      }
      ...
      if (load_modules) {
        ...
        for (const auto& module_entry : config["modules"]) {
          ...
          if (module_data["unique_id"]) {
            ...
            return Status::KeyError("No 'unique_id' specified for module " +
          ...
          if (module_data["states_update_default_rate"]) {
            ...
            if (rate < 1.0f) {
              return Status::Invalid(
                "Module '" + params.module_ros_name +
                "' has 'states_update_default_rate' less than 1Hz, which is not allowed");
            }
          ...
          if (module_data["states_custom_rates"]) {
            for (const auto& rate_entry : module_data["states_custom_rates"]) {
              ...
              if (rate <= 0.0f) {
                return Status::Invalid("Module '" + params.module_ros_name +
                                       "' has custom rate for state '" + state_name +
      ...
      return Status::ExpressionValidationError("YAML parsing error: " +
                                               std::string(e.what()));
    } catch (const std::exception& e) {
      return Status::ExpressionValidationError("Error loading params: " +
                                               std::string(e.what()));
    }
  }
  ...
  std::string get_can_primary_interface_name() const { return _can_primary_interface; }
  ...
  float get_global_default_rate() const { return _global_default_rate; }

  float get_max_update_rate() const { return _max_update_rate; }
 ...
 private:
  ...
  Result<uint8_t> get_new_node_id()
  {
    ...
    for (uint8_t candidate_id = 2; candidate_id < 255; ++candidate_id) {
