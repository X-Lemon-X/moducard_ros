...
public:
  ...
  /// @brief Stop the driver , this will be called when the driver is stopped or before cleanup.
  ///
  /// This should remove all the callbacks from the CAN interface and stop any ongoing work.
  HARDWARE_PUBLIC virtual Status driver_on_deactivate() = 0;

  /// @brief This will be called in a loop after the driver is activated, this is where the main control logic
  /// should be implemented.
  ///
  /// So this call should basically request appropriate hardware states with appropriate frequencies updates
  /// according to module config and send out commands to control using some queues from services
  ...
  HARDWARE_PUBLIC HardwareType_t driver_get_hardware_type() {
    return _hardware_type;
  }

  HARDWARE_PUBLIC DeviceIdentifier_t driver_get_device_identifier() {
...
public:
  Result<McSlavePluginDriver<McCanSlaveInterface, Hardware, McRosMsgTranslator> *> static Make(
  rclcpp::Node &node,
  std::shared_ptr<CanBase> can_interface_primary,
  std::shared_ptr<CanBase> can_interface_secondary,
  const ModuleParams &module_params) {
    ...
    if(module_params.node_id < 2 || module_params.node_id > 255) {
      return Status::Invalid("Node ID must be an 8-bit value 2-255 (0 and 1 are reserved)");
    }
    ...
    return Result<McSlavePluginDriver *>::OK(new McSlavePluginDriver(node, std::move(can_interface_primary),
  ...
  HARDWARE_PUBLIC virtual Status driver_on_init() override {
    ...
    [&](auto &&...args) {
      ...
      [&] {
        ...
        using RosMsgT =
        std::decay_t<decltype(McRosMsgTranslator::mcan_decode_message_to_ros(this->_interface.*(args)))>;
        auto msg_buffer = std::make_shared<CanMultiPackageFrame<MsgT>>();
        auto publisher  = this->create_publisher<RosMsgT>(_module_params.module_ros_name + "/get/" +
                                                          std::string(MsgT::k_group) + "/" + std::string(MsgT::k_name),
                                                          10);
        uint32_t can_id =
        mcan_connect_msg_id_with_node_id((this->_interface.*(args)).k_base_address, _module_params.node_id);

        auto can_callback = [this, args, msg_buffer, publisher](CanBase &can, const CanFrame &frame, void *arg) {
          (void)can;
          (void)arg;
    ...
    [&](auto &&...args) {
      ...
      [&] {
        ...
        using RosMsgT =
        std::decay_t<decltype(McRosMsgTranslator::mcan_decode_message_to_ros(this->_interface.*(args)))>;
        auto publisher = this->create_subscription<RosMsgT>(
        _module_params.module_ros_name + "/set/" + std::string(MsgT::k_group) + "/" + std::string(MsgT::k_name),
        10, [this, args](const RosMsgT &ros_msg) {
          if(!this->_is_active.load()) {
            RCLCPP_WARN(this->get_logger(), "Received command while driver is not active, ignoring");
            return;
          }
          auto cmd                         = McRosMsgTranslator::mcan_encode_ros_to_message(ros_msg);
          (this->_interface.*(args)).value = cmd.value;
          mcan_pack_send_msg(*_can_interface_primary, this->_interface.*(args), _module_params.node_id);
        });
      }(),
      ...);
    },
    ...
    [&](auto &&...args) {
      ...
      [&] {
        using MsgT = std::decay_t<decltype(this->_interface.*(args))>;
        auto maybe_override_rate =
        std::find_if(_module_params.states_custom_rates.begin(), _module_params.states_custom_rates.end(),
        ...
        if(maybe_override_rate != _module_params.states_custom_rates.end()) {
          RCLCPP_INFO(this->get_logger(), "Using custom update rate for state %s: %f Hz", MsgT::k_name,
    ...
    [&](auto &&...args) {
      ...
      [&] {
        ...
        if(callback == _can_callbacks.end()) {
          return Status::KeyError("No callback found for CAN ID: " + std::to_string(can_id));
        }
      ...
      [&] {
        if constexpr((this->_interface.*(args)).k_allow_read) {
          if(FrequencyTimer::should_trigger_and_reset(this->_timers[idx])) {
            mcan_request_msg(*_can_interface_primary, this->_interface.*(args), _module_params.node_id);
          }
        }
        ++idx;
      }(),
